...
def padding(data):
    ...
    # this code pad every example in the batch  to be the length of the longest example
    # but, in the running of code, some example's length is shorter than 28,
    # (28 is a number written in the code,don't know why
    src_len = [len(s) for s in src]
    temoo = max(max(src_len), 32)
    src_pad = torch.zeros(len(src), temoo).long()
    for i, s in enumerate(src):
        end = src_len[i]
        src_pad[i, :end] = torch.LongTensor(s[end-1::-1]) #why store the vector in the reverse order?

    tgt_len = [len(s) for s in tgt]
    tgt_pad = torch.zeros(len(tgt), max(max(tgt_len), 32)).long()
